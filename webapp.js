...
(() => {
  ...
  // depth-aware Dark Bloom, SM-306 low-frequency soft-history stabilization and
  // SM-307 bounded DSO/self/contact/Bloom/AO visibility composition with a
  // reserved GTAO consumer interface. Auto remains WebGL2 until SM-505; staged
  ...
  const backendReady = import('./engine/webgpu_device.js?v=sm102-1')
    ...
    .catch(err => {
      ...
      console.warn('WebGPU staged renderer lifecycle through post/occluder/cluster/dominance/DSO hierarchy/Dark Bloom/temporal/bounded visibility preparation unavailable; continuing with WebGL2.', err);
      return null;
    });
