...
(() => {
  ...
  // SM-102/103 platform/resource lifecycle plus the SM-200 Material-v2 G-buffer,
  // accepted SM-201 depth model and SM-202 hardware per-pixel ownership pass.
  // Auto intentionally remains WebGL2 until SM-505; loading these modules makes
  // the staged WebGPU representation path available without changing presentation.
  const backendReady = import('./engine/webgpu_device.js?v=sm102-1')
    ...
    .catch(err => {
      ...
      console.warn('WebGPU platform/resource/ownership lifecycle unavailable; continuing with WebGL2.', err);
      return null;
    });
