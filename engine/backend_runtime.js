...
(function(root,factory){
  ...
  const api=factory(deviceApi,root);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.SteelMothBackendRuntime=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(Device,root){
  ...
  class BackendRuntime{
    constructor(options={}){
      ...
      this.managerFactory=options.managerFactory||((opts)=>new Device.WebGPUDeviceManager(opts));this.canvasFactory=options.canvasFactory||(()=>this.documentRef?.createElement?.('canvas')||null);
    ...
    async select(mode,options={}){
      ...
      this.manager?.close?.();this.manager=null;this.probeCanvas=null;this.requestedBackend=mode;this.policy=Device.resolveBackendPolicy(mode);this.fallbackReason='';this.lastError=null;if(persist)this._persist(mode);
      ...
      try{
        ...
        if(generation!==this._generation)return this.diagnostics();this.lastError={name:String(error?.name||'Error'),message:String(error?.message||error)};this.fallbackReason=this.lastError.message;this.activeBackend=Device.BACKENDS.WEBGL2;this.presentationBackend=Device.BACKENDS.WEBGL2;this._setStatus('fallback');return this.diagnostics();
      }
    }
    _deviceLost(manager,info){if(manager!==this.manager)return;this.fallbackReason=`device-lost:${info?.reason||'unknown'}${info?.message?`: ${info.message}`:''}`;this.activeBackend=Device.BACKENDS.WEBGL2;this.presentationBackend=Device.BACKENDS.WEBGL2;this._setStatus('fallback-device-lost')}
    ...
    async resizeProbe(width,height,dpr=1){if(this.activeBackend!==Device.BACKENDS.WEBGPU||this.manager?.status!=='ready')return null;return this.manager.resize(width,height,dpr)}
    attachGame(game){
    ...
    updateControls(){
      ...
      if(status){const auto=this.requestedBackend===Device.BACKENDS.AUTO?'Auto is gated to WebGL2 until SM-505. ':'';const compat=this.activeBackend===Device.BACKENDS.WEBGPU?'WebGPU device/context ready; frame presentation remains WebGL2 compatibility during migration. ':'';const fallback=this.fallbackReason?`Fallback: ${this.fallbackReason}. `:'';status.textContent=`${auto}${compat}${fallback}requested=${this.requestedBackend} · active=${this.activeBackend} · presentation=${this.presentationBackend}`}
    }
    diagnostics(){return {schema:'steelmoth-backend-runtime/v1',requestedBackend:this.requestedBackend,activeBackend:this.activeBackend,presentationBackend:this.presentationBackend,status:this.status,selectionSource:this.selectionSource,policy:clone(this.policy),autoWebGPUEnabled:Device.AUTO_WEBGPU_ENABLED,autoPolicy:Device.AUTO_POLICY,fallbackReason:this.fallbackReason,lastError:this.lastError?{...this.lastError}:null,lastTransitionAt:this.lastTransitionAt,webgpu:this.manager?.diagnostics?.()||null,note:'SM-102 initializes WebGPU platform state only; rendering remains WebGL2 compatibility until later WebGPU rendering tasks.'}}
    ...
    close(){this._generation++;this.manager?.close?.();this.manager=null;this.probeCanvas=null;this._setStatus('closed')}
  }
