...
(function(root,factory){
  ...
  function classificationFor(instance={}){
    ...
    if(category==='GROUND_MOSS'||category==='SHORT_GRASS'||h<14)return{role:'receiver-only',flags:CLASS_FLAGS.receiver|CLASS_FLAGS.tiny,macroEligible:false,contactEligible:false};
    if(category==='FERN'||category==='FLOWER_CLUSTER'||h<28)return{role:'contact-receiver',flags:CLASS_FLAGS.receiver|CLASS_FLAGS.contact,macroEligible:false,contactEligible:true};
    const macro=(category==='BUSH'||category==='BROAD_LEAF')&&h>=28;
  ...
  class FoliageFrameAuthority{
    constructor(manifest={}){this.registry=new Foliage.FoliageRegistry(manifest);this.interaction=new Foliage.InteractionField();this.depth=new Foliage.DepthClassifier();this.instances=[];this.signature='';this.quality=-1;this.time=0;this.last={foregroundCount:0,backgroundCount:0,dirty:false};}
    setDescriptor(descriptor={},settings={}){const s=normalizeSettings(settings),key=`${descriptor?.signature||''}|q:${s.foliageQuality}`;if(key===this.signature)return false;this.instances=Foliage.generateFoliageInstances(descriptor,this.registry,s.foliageQuality);this.depth.setInstances(this.instances);this.signature=key;this.quality=s.foliageQuality;return true;}
    update(dt,actors=[],settings={},time=null){const s=normalizeSettings(settings);if(s.foliageQuality!==this.quality&&this.signature)this.signature='';this.time=Number.isFinite(time)?Number(time):this.time+Math.max(0,finite(dt));const safeActors=copyActorSources(actors),sources=this.interaction.update(Math.max(0,finite(dt)),safeActors,s.foliageQuality);this.last=this.depth.update(Math.max(0,finite(dt)),sources);return{instances:this.instances,sources:this.interaction.bounded(),time:this.time,depth:{...this.last}};}
...
fn mainLight(i:Instance,p:vec2f,z:f32)->f32{var e=0.0;if(params.debugQuality.z==0u){return 0.0;}for(var k=0u;k<params.sizeCount.w;k++){let Ld=lights[k].posRadius.xyz-vec3f(p.x,-p.y,z);let d=length(vec2f(Ld.x,Ld.y));let r=max(lights[k].posRadius.w,1.0);if(d>r){continue;}let L=normalize(Ld);let wind=sin((i.rootSize.x*.88+i.rootSize.y*.26)/216.0*6.2831853+params.timeWind.x*(.34+.34*params.timeWind.z))*params.timeWind.y;let bend=wind*i.profile.x*params.shadeAmbient.x*2.1;let nx=clamp(-bend*.055,-.65,.65);let nz=sqrt(max(.001,1.0-nx*nx));let ndl=max(0.0,nx*L.x+nz*L.z);let lum=dot(lights[k].colorIntensity.rgb,vec3f(.2126,.7152,.0722));let a=exp(-2.3*(d/r)*(d/r));e+=lum*lights[k].colorIntensity.w*a*(.26+.74*ndl);}return e;}
@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) gid:vec3u){let idx=gid.x;if(idx>=params.sizeCount.z){return;}let i=instances[idx];let px=clamp(i32(round(i.rootSize.x)),0,i32(params.sizeCount.x)-1);let py=clamp(i32(round(i.rootSize.y)),0,i32(params.sizeCount.y)-1);let v=textureLoad(visibilityTex,vec2i(px,py),0);let dep=textureLoad(depthTex,vec2i(px,py),0);let directVis=sat(v.y);let ambientVis=sat(v.z);let direct=mainLight(i,i.rootSize.xy,i.rootSize.w*.18);let illum=select(1.0,clamp(params.shadeAmbient.y*ambientVis+direct*directVis*params.timeWind.w,0.0,2.0),params.debugQuality.z!=0u);let wind=sin((i.rootSize.x*.88+i.rootSize.y*.26)/216.0*6.2831853+params.timeWind.x*(.34+.34*params.timeWind.z))*params.timeWind.y;let bend=wind*i.profile.x*params.shadeAmbient.x*2.1;output[idx].color=vec4f(i.base.rgb*illum,i.base.a);output[idx].state=vec4f(illum,directVis,ambientVis,dep);output[idx].ids=vec4u(i.ids.x,i.ids.y,bitcast<u32>(bend),bitcast<u32>(i.meta.y));}
`;
