...
(function(root,factory){
  ...
  function resolveFormats(options={}){const formats={...FORMATS,...(options.formats||{})},normalEncoding=options.normalEncoding==='oct8'?'oct8':'xyz';if(!['rgba16float','rgba8unorm'].includes(formats.g1))throw new Error(`unsupported experimental G1 format ${formats.g1}`);if(!['rgba16float','rgba8unorm'].includes(formats.g2))throw new Error(`unsupported experimental G2 format ${formats.g2}`);if(normalEncoding==='oct8'&&formats.g1!=='rgba8unorm')throw new Error('oct8 normal encoding requires rgba8unorm G1');for(const key of ['g0','objectId','depth'])if(formats[key]!==FORMATS[key])throw new Error(`SM-800 cannot change invariant ${key} format`);return{formats:Object.freeze(formats),normalEncoding}}
  function materialWgsl(normalEncoding='xyz'){if(normalEncoding!=='oct8')return MATERIAL_WGSL;return MATERIAL_WGSL.replace('struct FSOut {','fn smOctEncode(n0:vec3f)->vec2f{var n=n0/(abs(n0.x)+abs(n0.y)+abs(n0.z));if(n.z<0.0){let old=n.xy;n.x=(1.0-abs(old.y))*select(-1.0,1.0,old.x>=0.0);n.y=(1.0-abs(old.x))*select(-1.0,1.0,old.y>=0.0);}return n.xy*.5+.5;}\nstruct FSOut {').replace('out.g1=vec4f(n*.5+.5,nr.a);','out.g1=vec4f(smOctEncode(n),0.0,nr.a);')}
...
@fragment fn fs_main(in:VSOut)->FSOut{
  ...
  let mode=in.material.w;var base:vec3f;if(mode<.5){base=mix(t.rgb,t.rgb*in.tintAlpha.rgb,.07);}else if(mode<1.5){base=mix(t.rgb,in.tintAlpha.rgb,clamp(in.material.z,0.0,1.0));}else{let lum=dot(t.rgb,vec3f(.26,.62,.12));base=in.tintAlpha.rgb*(.28+lum*.98)+max(t.rgb,vec3f(0))*.12;}
...
@fragment fn fs_main(@builtin(position) p:vec4f)->@location(0) vec4f{let q=vec2i(p.xy);let a=textureLoad(g0,q,0);let n=textureLoad(g1,q,0);let m=textureLoad(g2,q,0);let id=textureLoad(oid,q,0).x;if(debug.mode==0u){return vec4f(a.rgb,1);}if(debug.mode==1u){return vec4f(n.xyz,1);}if(debug.mode==2u){return vec4f(vec3f(n.a),1);}if(debug.mode==3u){return vec4f(vec3f(m.r),1);}if(debug.mode==4u){return vec4f(vec3f(m.g),1);}if(debug.mode==5u){return vec4f(vec3f(m.b),1);}if(debug.mode==6u){return vec4f(vec3f(m.a),1);}let h=f32((id^(id>>8u)^(id>>16u))&255u)/255.0;return vec4f(h,fract(h*5.17),fract(h*11.31),1);}`;
  ...
  class WebGPUMaterialGBuffer{
    ...
    async _readTexturePixel(record,x,y,kind){const bytesPerPixel=kind==='rgba16float'?8:4,buffer=this.device.createBuffer({label:`${this.labelPrefix}:readback:${kind}`,size:256,usage:bufferUsage(['COPY_DST','MAP_READ'])}),encoder=this.device.createCommandEncoder();encoder.copyTextureToBuffer({texture:record.handle,origin:{x:Math.max(0,Math.min(record.width-1,Math.floor(x))),y:Math.max(0,Math.min(record.height-1,Math.floor(y))),z:0}},{buffer,bytesPerRow:256,rowsPerImage:1},{width:1,height:1,depthOrArrayLayers:1});this.queue.submit([encoder.finish()]);await buffer.mapAsync(Number(root?.GPUMapMode?.READ??MAP_MODE_READ));const raw=new Uint8Array(buffer.getMappedRange()).slice(0,bytesPerPixel);buffer.unmap();buffer.destroy();if(kind==='rgba8')return Array.from(raw);if(kind==='rgba8float')return Array.from(raw,v=>v/255);if(kind==='r32uint')return new DataView(raw.buffer,raw.byteOffset,4).getUint32(0,true);const dv=new DataView(raw.buffer,raw.byteOffset,raw.byteLength);return [0,2,4,6].map(o=>halfToFloat(dv.getUint16(o,true)))}
    async readPixel(x,y){return {g0:await this._readTexturePixel(this._record('g0'),x,y,'rgba8'),g1:await this._readTexturePixel(this._record('g1'),x,y,this.formats.g1==='rgba8unorm'?'rgba8float':'rgba16float'),g2:await this._readTexturePixel(this._record('g2'),x,y,this.formats.g2==='rgba8unorm'?'rgba8float':'rgba16float'),objectId:await this._readTexturePixel(this._record('object'),x,y,'r32uint')};}
    diagnostics(){return {schema:SCHEMA,formats:{...this.formats},normalEncoding:this.normalEncoding,clear:clone(CLEAR),extent:{width:this.width,height:this.height},alphaCutoff:this.alphaCutoff,maxInstances:this.maxInstances,renderCount:this.renderCount,lastInstanceCount:this.lastInstanceCount,atlas:this.atlas?{width:this.atlas.width,height:this.atlas.height,formats:{...this.atlas.formats}}:null,colourSpace:{albedoSource:'srgb-external-image -> rgba8unorm-srgb -> sampled-linear',tints:'authored-srgb -> cpu-linear',g0:'linear-albedo-rgba8unorm',g1:'linear-normal-roughness',g2:'linear-height-metalness-ao-emissive',debug:'raw-linear/data values; presentation must display-transform'},compilation:clone(this.compilation),debugModes:[...DEBUG_MODES],resourceDiagnostics:this.registry.diagnostics(),pipelineDiagnostics:this.pipelines.diagnostics(),materialCompatibility:{alphaCutoff:.12,normalTintStrength:.07,flatDefaultTintStrength:.18,tintMode:'compatibility-luminance'},ownershipDepth:'not-implemented-sm201-sm202',depthAttachmentPolicy:'clear-only; depth compare always and writes disabled'};}
    close(){this._destroyAtlases();if(this.ownsRegistry)this.registry.close();this.debugPipelines.clear();}
  }

  return {SCHEMA,INSTANCE_STRIDE,FORMATS,CLEAR,CATEGORY_ORDER,MATERIAL_MODE,DEBUG_MODES,MATERIAL_WGSL,DEBUG_WGSL,resolveFormats,materialWgsl,objectIdForStableId,regionRect,compatibilityHeightFactor,srgbChannelToLinear,srgbToLinear,buildSceneInstances,packInstances,halfToFloat,WebGPUMaterialGBuffer};
});
