...
(function(root,factory){
  ...
  function resolveQuality(value='Medium',overrides={}){const name=normalizeQuality(value),base=QUALITY_PRESETS[name];return{schema:`${SCHEMA}/quality`,name,gtao:{...clone(base.gtao),...(overrides.gtao||{})},temporal:{...clone(base.temporal),...(overrides.temporal||{})}};}
...
@compute @workgroup_size(8,8) fn cs_main(@builtin(global_invocation_id) gid:vec3u){
  ...
  class WebGPUGTAOTemporal{
    ...
    async _pipeline(){if(this.pipeline)return this.pipeline;this.pipeline=await this.pipelines.getCompute('gtao-temporal-v2',async(device,label)=>{const module=device.createShaderModule({label:`${label}:wgsl`,code:TEMPORAL_WGSL});if(typeof module.getCompilationInfo==='function'){const info=await module.getCompilationInfo(),errors=(info.messages||[]).filter(m=>m.type==='error');if(errors.length)throw new Error(`SM-601 WGSL compilation failed: ${errors.map(e=>e.message).join('; ')}`);}return device.createComputePipeline({label,layout:'auto',compute:{module,entryPoint:'cs_main'}});});return this.pipeline;}
    ...
    bindings(){return{gtaoVisibility:this._record(`vis${this.historyIndex}`).handle.createView(),visibility:this._record(`vis${this.historyIndex}`).handle.createView(),format:'r32float',generation:this.generation,quality:this.quality};}
    ...
    diagnostics(){const quality=resolveQuality(this.quality);return{schema:SCHEMA,valid:this.valid,historyValid:this.historyValid,generation:this.generation,updateCount:this.updateCount,invalidationCount:this.invalidationCount,lastInvalidationReason:this.lastInvalidationReason,extent:{width:this.width,height:this.height},quality,previousMeta:this.previousMeta?{...this.previousMeta}:null,lastStats:this.lastStats?{...this.lastStats}:null,consumer:'SM-307 gtaoVisibilityView',historyPolicy:'Depth/object/normal/room/device/backend discontinuities reject history; accepted history is neighborhood and delta clamped.',targetHardwareEvidence:false,storageTextureCount:4,resourceDiagnostics:this.registry?.diagnostics?.()||null,pipelineDiagnostics:this.pipelines?.diagnostics?.()||null};}
