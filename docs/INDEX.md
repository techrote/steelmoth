...
- [`WEBGPU_LIGHTING_SM204.md`](WEBGPU_LIGHTING_SM204.md) — SM-204 one canonical point/omni/cone light buffer, deferred Material-v2 GGX response, diagnostic preset, debug modes and eight-angle real-WebGPU validation contract.
...
- `.github/workflows/verification.yml` — hosted source/regression, required real-WebGPU WGSL/pipeline/resource/G-buffer/ownership/depth-hierarchy/lighting validation, WebGL2 pixel parity, GLSL/MRT software, and clean-package verification.
...
SM-204 ports the restrained v1.2.3 direct-light response onto WebGPU without changing normal/material semantics. Point, player-omni and player-cone records are normalized into one persistent 64-byte-per-light buffer; one deferred WGSL pass consumes that buffer and Material-v2 G0/G1/G2 for Schlick/GGX/Smith diffuse/specular plus compatibility ambient/emissive response. Diffuse, specular, final and active-light-count readbacks are executable, and the real-WebGPU gate checks eight light angles plus X/Y normal orientation. SM-205 remains responsible for height self/contact visibility; procedural consumers are still downstream.

The root/foot authority, per-pixel ownership representation, shared depth hierarchy and canonical opaque-light representation are therefore implemented prerequisites for later shadow clustering/DSO and procedural integration. Downstream systems must consume these resolved contracts rather than treating G2.R local height or backend-specific uniforms as competing authority.
