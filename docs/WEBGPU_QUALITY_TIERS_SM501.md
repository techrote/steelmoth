...
The two later positions-6/7 failures were benchmark lifecycle accumulation, not scene-intrinsic renderer regressions. At clean harness source `eb4c37599b188a21f52602d4071ed4c7ac699027`, fresh-process isolation measured bin-cluster at 3.920/5.034 ms mean/p95, diagnostic-light at 9.508/11.728 ms, and dense-static control at 11.221/13.100 ms. The benchmark had created a complete WebGPU device/resource stack per page without explicit release before navigating within a session. It now drains instrumentation/queue work, closes all renderer resource owners and image bitmaps, unconfigures the canvas, and destroys the device. A source-matched one-process confirmation then measured bin-cluster in position 6 at 3.944/4.806 ms and diagnostic-light in position 7 at 9.984/12.863 ms, with every page reporting successful teardown and device status `closed`.

The required full rerun at clean source `5cf74587c69c26e06d734eccfe1a6cb98de2d46d` confirms that both scoped failures are repaired across three sessions: bin-cluster is 4.564/5.813 ms aggregate mean/p95 and diagnostic-light is 10.498/13.636 ms. Full acceptance nevertheless still **fails** three strict requirements: dense-static is 12.294/15.971 ms, exceeding both limits, and mixed p95 is 14.561 ms, exceeding 14.5 ms while its 11.491 ms mean passes. Firefox representative passes at 7.712/9.353 ms. No CPU time was subtracted and no shader-only time is inferred.

**SM-501, issue #31 and PR #92 remain open and unmerged.** SM-501 must remain open until a physical GTX 1650 SUPER Medium benchmark dataset satisfying the protocol above is committed/reviewed, together with the Firefox target-machine spot-check, and every canonical scenario meets the acceptance limits. The current source-matched dataset completes the required protocol but fails dense-static mean/p95 and mixed p95. A hosted software/fallback adapter result, a targeted diagnostic pass, or a large improvement in other scenes cannot close this gate. Complete before/after samples, screenshots, teardown records, reports and strict-validator output are under `benchmarks/webgpu-gtx1650s/post-sm501-dark-bloom-fix-2026-09-20/`.
...
`--phase sm501-diagnostic --scenes ...` provides a non-acceptance total-only route for explicit scene selection with a fresh Chrome process per scene. `--reuse-process` preserves one Chrome process across those selected scenes for sequential lifecycle diagnosis. `sm501-localization` additionally exports the full per-pass `cpuCallbackMs` distributions beside `passGpuMs`.
