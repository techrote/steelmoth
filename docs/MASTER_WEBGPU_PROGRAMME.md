...
Recover the original box/bin screenshots if available and commit them as immutable references. Create deterministic renderer fixtures for boxes, binsright, binsleft, binsupleft, binsup, crate, barrel, cabinet, pipes, lamp, robot, foliage, water and dense mixed terrain.

### SM-002 — Build deterministic capture/diagnostics harness

Add query/CLI-accessible deterministic render-test mode producing screenshot PNGs, diagnostics JSON and debug-buffer dumps for named fixtures/light angles/backends.
...
### SM-405 — Cross-browser WebGPU release gate

Chrome + Firefox on target Windows hardware: API correctness, visual fixtures, fallback behaviour and no black-canvas failure. Promote `Auto` to prefer WebGPU only after gates pass.

## M5 — Performance architecture and lighting foundation
...
Quarter-resolution depth-aware cone scattering with low-frequency atmospheric density and temporal accumulation. Must remain faint atmospheric scattering, not a thick fog cone.

### SM-701 — Volumetric occlusion/stability/performance

Reuse scene depth/DSO, reject temporal trails and characterize the GPU budget.
