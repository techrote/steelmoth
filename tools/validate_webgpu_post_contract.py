...
need("c=vec3(1.0)-exp(-max(c,0.0)*uExposure)" in game and "pow(max(c,0.0),vec3(.96))" in game,'baseline exposure/output transfer changed unexpectedly')
...
need("scene -> output; bloom/grade/post skipped" in src,'raw/debug post bypass must be explicit')
need("colour-space redesign deferred to SM-502" in src,'SM-502 colour-space deferral must remain explicit')
...
if errors:
    ...
    raise SystemExit(1)
print('SM-207 post contract PASS: baseline post/persistence, WebGPU bloom/post/raw/output and SM-502 boundary are coherent')
