...
def validate(smoke,require_webgpu):
    if not smoke.get('ok'):raise RuntimeError(smoke.get('error','SM-301 cluster page reported failure'))
    checks=smoke.get('checks') or []
    if len(checks)<20:raise RuntimeError(f'insufficient SM-301 browser assertions: {len(checks)}')
    failed=[c for c in checks if not c.get('ok')]
    if failed:raise RuntimeError(f'SM-301 smoke contains failed checks: {failed[:2]}')
    diag=smoke.get('diagnostics') or {}
    if diag.get('schema')!='steelmoth-webgpu-clusters/v1':raise RuntimeError(f"unexpected diagnostics schema: {diag.get('schema')}")
    ...
    if require_webgpu and not all(k in readback for k in ('firstClusterId','firstClusterMembers','firstMember')):raise RuntimeError('required GPU cluster/member readback evidence missing')
def main():
    ap=argparse.ArgumentParser(description='Real-browser WebGPU stable cluster validation for SM-301.')
    ap.add_argument('--report',type=Path,default=Path('artifacts/webgpu-clusters-browser.json'));ap.add_argument('--timeout',type=float,default=190);ap.add_argument('--require-webgpu',action='store_true');args=ap.parse_args()
    ...
    report={'schema':'steelmoth-webgpu-clusters-browser-report/v1','ok':False,'browserExecutable':exe,'requireWebGPU':args.require_webgpu,'evidenceBoundary':'Real hosted WebGPU validates deterministic geometry-only SM-301 cluster/member buffers, bounded tile-local candidate work, perturbation stability, invalidation and GPU readback. It does not select light-dependent dominant owners or generate DSO shadows.'}
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='steelmoth-sm301-chrome-') as profile:
            ...
            while time.monotonic()<deadline and proc.poll() is None:
                try:
                    target=next((t for t in json_get(f'http://127.0.0.1:{debug}/json/list') if t.get('type')=='page' and 'webgpu-clusters-smoke.html' in t.get('url','')),None)
                    if target:break
                except Exception:pass
                time.sleep(.15)
            if not target:raise RuntimeError('Chrome DevTools SM-301 page target did not become available')
            ...
            while time.monotonic()<deadline:
                if cdp.eval("document.body && document.body.dataset.webgpuClusterDone==='1'"):break
                time.sleep(.2)
            else:raise RuntimeError('SM-301 cluster page did not publish before timeout')
            text=cdp.eval("document.getElementById('webgpuClusterResult')?.textContent || ''")
            if not text:raise RuntimeError('webgpuClusterResult missing after readiness signal')
            smoke=json.loads(text);report['smoke']=smoke;report['browserCommand']=cmd[:-1]+['<local-sm301-url>'];validate(smoke,args.require_webgpu);report['ok']=True
    ...
    path.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n',encoding='utf-8');smoke=report.get('smoke') or {};print(f"SM-301 WEBGPU CLUSTERS {'PASS' if report['ok'] else 'FAIL'}: browser={exe} checks={len(smoke.get('checks') or [])}")
