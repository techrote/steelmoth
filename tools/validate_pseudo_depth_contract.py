...
def forbid(haystack:str,needle:str,label:str)->None:
    if needle in haystack: raise AssertionError(f'{label}: forbidden duplicate/production ownership term {needle!r}')
...
def main()->int:
    ...
    require(gbuffer,"ownershipDepth:'not-implemented-sm201-sm202'",'SM-200 production boundary')
    require(gbuffer,"depthWriteEnabled:false,depthCompare:'always'",'SM-200 production boundary')
    ...
    print('Pseudo-depth source contract PASS: one reference formula, SM-200 depth writes still disabled, SM-202 remains production owner.')
    return 0
