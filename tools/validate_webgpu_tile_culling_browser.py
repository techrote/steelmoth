...
def validate(smoke):
    if not smoke.get('ok'):raise RuntimeError(smoke.get('error','SM-504 browser smoke failed'))
    checks=smoke.get('checks') or []
    if len(checks)<40:raise RuntimeError(f'insufficient SM-504 browser assertions: {len(checks)}')
    failed=[c for c in checks if not c.get('ok')]
    if failed:raise RuntimeError(f'SM-504 smoke contains failed checks: {failed[:2]}')
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='steelmoth-sm504-chrome-') as profile:
            ...
            while time.monotonic()<deadline:
                if cdp.eval("document.body && document.body.dataset.webgpuTileCullingDone==='1'"):break
                time.sleep(.2)
            else:raise RuntimeError('SM-504 page did not publish before timeout')
            text=cdp.eval("document.getElementById('webgpuTileCullingResult')?.textContent || ''");smoke=json.loads(text);report['smoke']=smoke;report['browserCommand']=cmd[:-1]+['<local-sm504-url>'];validate(smoke);report['ok']=True
    ...
    path.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n',encoding='utf-8');smoke=report.get('smoke') or {};print(f"SM-504 WEBGPU TILE CULLING {'PASS' if report['ok'] else 'FAIL'}: browser={exe} checks={len(smoke.get('checks') or [])}")
