...
def validate(smoke,require_webgpu):
    ...
    if len(smoke.get('checks') or [])<32:raise RuntimeError(f"insufficient lighting assertions: {len(smoke.get('checks') or [])}")
...
def main():
    ap=argparse.ArgumentParser(description='Real-browser canonical WebGPU light-buffer and deferred Material-v2 PBR validation for SM-204.');ap.add_argument('--report',type=Path,default=Path('artifacts/webgpu-lighting-browser.json'));ap.add_argument('--timeout',type=float,default=120);ap.add_argument('--require-webgpu',action='store_true');a=ap.parse_args();exe=browser();path=a.report if a.report.is_absolute() else ROOT/a.report;path.parent.mkdir(parents=True,exist_ok=True);report={'schema':'steelmoth-webgpu-lighting-browser-report/v1','ok':False,'browserExecutable':exe,'requireWebGPU':a.require_webgpu,'evidenceBoundary':'Real hosted WebGPU canonical-light buffer, eight-angle deferred Material-v2 PBR, normal-axis and debug readback correctness. Not target-GPU performance, DSO/self/contact shadow parity, procedural integration, or final product visual approval.'}
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='steelmoth-sm204-chrome-') as profile:
            ...
            smoke=json.loads(text);report['smoke']=smoke;report['browserCommand']=cmd[:-1]+['<local-sm204-url>'];validate(smoke,a.require_webgpu);report['ok']=True
    ...
    path.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n',encoding='utf-8');smoke=report.get('smoke') or {};print(f"SM-204 WEBGPU LIGHTING {'PASS' if report['ok'] else 'FAIL'}: browser={exe} checks={len(smoke.get('checks') or [])} angles={len(smoke.get('angles') or [])}")
