...
def validate(smoke,require_webgpu):
    if not smoke.get('ok'):raise RuntimeError(smoke.get('error','SM-207 post page reported failure'))
    checks=smoke.get('checks') or []
    if len(checks)<18:raise RuntimeError(f'insufficient SM-207 browser assertions: {len(checks)}')
    failed=[c for c in checks if not c.get('ok')]
    if failed:raise RuntimeError(f'SM-207 smoke contains failed checks: {failed[:2]}')
    diag=smoke.get('diagnostics') or {}
    if diag.get('schema')!='steelmoth-webgpu-post/v1':raise RuntimeError(f"unexpected diagnostics schema: {diag.get('schema')}")
    ...
    if require_webgpu and not smoke.get('preferredCanvasFormat'):raise RuntimeError('required browser canvas evidence missing')
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='steelmoth-sm207-chrome-') as profile:
            ...
            while time.monotonic()<deadline and proc.poll() is None:
                try:
                    target=next((t for t in json_get(f'http://127.0.0.1:{debug}/json/list') if t.get('type')=='page' and 'webgpu-post-smoke.html' in t.get('url','')),None)
                    if target:break
                except Exception:pass
                time.sleep(.15)
            if not target:raise RuntimeError('Chrome DevTools SM-207 page target did not become available')
            ...
            while time.monotonic()<deadline:
                if cdp.eval("document.body && document.body.dataset.webgpuPostDone==='1'"):break
                time.sleep(.2)
            else:raise RuntimeError('SM-207 post page did not publish before timeout')
            text=cdp.eval("document.getElementById('webgpuPostResult')?.textContent || ''")
            if not text:raise RuntimeError('webgpuPostResult missing after readiness signal')
            smoke=json.loads(text);report['smoke']=smoke;report['browserCommand']=cmd[:-1]+['<local-sm207-url>'];validate(smoke,args.require_webgpu);report['ok']=True
    ...
    path.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n',encoding='utf-8');smoke=report.get('smoke') or {};print(f"SM-207 WEBGPU POST {'PASS' if report['ok'] else 'FAIL'}: browser={exe} checks={len(smoke.get('checks') or [])}")
