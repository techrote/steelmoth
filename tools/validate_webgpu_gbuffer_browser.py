...
def main()->int:
    ...
    report={'schema':'steelmoth-webgpu-gbuffer-browser-report/v1','ok':False,'browserExecutable':exe,'requireWebGPU':args.require_webgpu,'evidenceBoundary':'Production WebGPU G-buffer format/WGSL/readback correctness on the reported hosted browser adapter; not target-GPU performance or final SM-201/202 ownership depth.'}
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='steelmoth-sm200-chrome-') as profile:
            ...
            smoke=json.loads(text);report['smoke']=smoke;report['browserCommand']=cmd[:-1]+['<local-sm200-url>'];validate(smoke,args.require_webgpu);report['ok']=True
